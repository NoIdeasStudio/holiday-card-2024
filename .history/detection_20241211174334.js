  ...
  if (results.multiHandLandmarks) {
    ...
    results.multiHandLandmarks.forEach((landmarks, handIndex) => {
      ...
      const customLabels = [
        ...
        "c",       // Label for index 5
        "d",      // Label for index 9
        "e",        // Label for index 13
        "Little MCP"       // Label for index 17
      ];
      ...
      //   positionLandmarkDiv(mcp, mcpDivId, `mcp-${handIndex}-${mcpIndex}`, ` ${mcpIndex + 1}`);
      // });

      results.multiHandLandmarks.forEach((landmarks, handIndex) => {
  // Iterate over the MCP points (indices 1, 5, 9, 13, 17)
  [1, 5, 9, 13, 17].forEach((index, mcpIndex) => {
    const mcp = landmarks[index];  // Get the MCP landmark at the specific index
    const mcpDivId = `mcp-div-${handIndex}-${mcpIndex}`;  // Generate a unique ID for each MCP point
    const label = customLabels[mcpIndex];  // Get the corresponding label from the customLabels array

    // Pass the custom label to the function
    positionLandmarkDiv(mcp, mcpDivId, `mcp-${handIndex}-${mcpIndex}`, label);  // Call the function with the custom label
  });
});

function positionLandmarkDiv(landmark, divId, landmarkId, label) {
  const div = document.getElementById(divId);
  if (div) {
    // Assuming landmark.x and landmark.y are normalized coordinates between 0 and 1
    div.style.position = 'absolute';
    div.style.left = `${landmark.x * window.innerWidth}px`;  // Convert normalized x to pixel value
    div.style.top = `${landmark.y * window.innerHeight}px`;  // Convert normalized y to pixel value
    div.id = landmarkId;  // Set the landmark-specific ID
    div.className = 'mcp-point';  // Set the class for styling purposes

    // Use the custom label passed as argument
