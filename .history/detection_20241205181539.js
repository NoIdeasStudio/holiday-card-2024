  ...
  if (results.multiHandLandmarks) {
    results.multiHandLandmarks.forEach((landmarks, handIndex) => {
      landmarks.forEach((landmark, index) => {
        // Create a div for each landmark
        const landmarkDiv = document.createElement('div');
        
        // Set the div's position based on the landmark coordinates
        const x = landmark.x * window.innerWidth;  // Convert normalized x to screen width
        const y = landmark.y * window.innerHeight; // Convert normalized y to screen height

        // Position the div absolutely at the landmark coordinates
        landmarkDiv.style.position = 'absolute';
        landmarkDiv.style.left = `${x - 5}px`; // Subtract to center the div
        landmarkDiv.style.top = `${y - 5}px`;  // Subtract to center the div

        // Style the div (as a red dot, for example)
        landmarkDiv.style.width = '10px';
        landmarkDiv.style.height = '10px';
        landmarkDiv.style.backgroundColor = '#FF0000';
        landmarkDiv.style.borderRadius = '50%'; // Make it circular

        // Add a custom ID or class for styling/interaction
        landmarkDiv.id = `landmark-${handIndex}-${index}`; // Unique ID per landmark
        landmarkDiv.classList.add('landmark'); // Optional: Class for general styling

        // Append the landmark div to the body (or a container)
