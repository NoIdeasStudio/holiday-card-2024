...
positionLandmarkDiv(tip5, tipDivId5, `tip-${handIndex}-5`, "5");
  ...
  const distance = calculateDistance(
    thumbTip.x * window.innerWidth, thumbTip.y * window.innerHeight,
    indexTip.x * window.innerWidth, indexTip.y * window.innerHeight
  );
  ...
  if (distance <= 50) {
    ...
    // Optionally reset the background color if the distance is greater than 50px
    document.querySelector('canvas').style.backgroundColor = ''; // or any default color
  }
