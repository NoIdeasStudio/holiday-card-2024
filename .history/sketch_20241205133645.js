...
function setup() {
  ...
  // Create video capture in portrait mode (height > width)
  let videoSize = 1024; // Use a square size for the video capture
  video = createCapture(VIDEO, { flipped: true });
  video.size(videoSize, videoSize); // Keep the video square
  video.hide();
...
function draw() {
  ...
  // Rotate the canvas 90 degrees for portrait mode
  push();
  translate(width / 2, height / 2); // Move to the center
  rotate(HALF_PI); // Rotate 90 degrees clockwise (portrait)
  imageMode(CENTER);
  image(video, 0, 0); // Draw the video without stretching
  pop();

  // Draw the fingers graphics
  image(fingers, 0, 0);
...
function windowResized() {
  ...
  // Reposition and resize the video to fit the portrait format
  let videoSize = 1024; // Same video size
  video.size(videoSize, videoSize); // Keep it square
  video.position((windowWidth - videoSize) / 2, 0); // Center horizontally
}
