...
const videoElement = document.getElementsByClassName('input_video')[0]; // Get the video element
const canvasElement = document.createElement('canvas'); // Create an invisible canvas to overlay the video feed
document.body.appendChild(canvasElement); // Append it to the body (it won't be visible unless we draw on it)
const canvasCtx = canvasElement.getContext('2d'); // Get the drawing context for the canvas

let hands = [];

// Function to handle the results from the MediaPipe Hands model
function onResults(results) {
  ...
  canvasCtx.clearRect(0, 0, canvasElement.width, canvasElement.height); // Clear the previous frame
  canvasCtx.drawImage(results.image, 0, 0, canvasElement.width, canvasElement.height); // Draw the video frame

  // Check if hands are detected and draw landmarks and connections
  if (results.multiHandLandmarks) {
    for (const landmarks of results.multiHandLandmarks) {
      // Draw the hand connections
      drawConnectors(canvasCtx, landmarks, HAND_CONNECTIONS, { color: '#00FF00', lineWidth: 5 });
      // Draw the hand landmarks (tips and knuckles)
      drawLandmarks(canvasCtx, landmarks, { color: '#FF0000', lineWidth: 2 });
    }
  }
}
...
// Set the options for the hand detection model
...
// Set the callback function to handle results
handsModel.onResults(onResults);

// Initialize the camera
const camera = new Camera(videoElement, {
  onFrame: async () => {
    // Send the current video frame to the hands model for processing
