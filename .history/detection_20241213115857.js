...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Set stroke-dasharray and stroke-dashoffset dynamically based on the path length
  path.style.strokeDasharray = pathLength;  // Make the dash length the same as the path
  path.style.strokeDashoffset = pathLength;  // Hide the path initially

  const arrowhead = document.querySelector("#arrowhead");

  // Use the path's length to animate the arrowhead along the path
  arrowhead.style.offsetPath = `path("${path.getAttribute("d")}")`;
  arrowhead.style.offsetDistance = "0%";
});
