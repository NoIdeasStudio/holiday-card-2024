...
function onResults(results) {
  ...
  if (results.multiHandLandmarks) {
    ...
    results.multiHandLandmarks.forEach((landmarks, handIndex) => {
      ...
      const wristDivId = `wrist-div-${handIndex}`;
      positionLandmarkDiv(wrist, wristDivId, `wrist-${handIndex}`, "f", false);

      // Handle MCP points (indices 1, 5, 9, 13, 17)
      for (let i = 1; i <= 5; i++) {
        const mcp = landmarks[i * 4];
        const mcpDivId = `mcp-div-${handIndex}-${i}`;
        positionLandmarkDiv(mcp, mcpDivId, `mcp-${handIndex}-${i}`, `${i}`, false);
      }
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        positionLandmarkDiv(tip, tipDivId, `tip-${handIndex}-${i}`, `${i}`, false);
      }
