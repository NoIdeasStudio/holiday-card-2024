...
//       canvasElement.style.backgroundColor = '#FF0000'; // Reset to red
//     }
//   }
...
function onResults(results) {
  ...
  function checkDistances(landmarks) {
    ...
    if (distances[0].dist < 50) {
      ...
      // Show only the closest pair of points
      positionLandmarkDiv(distances[0].points[0], 'closest-point-1', 'closest-point', '1');
      positionLandmarkDiv(distances[0].points[1], 'closest-point-2', 'closest-point', '2');
    } else {
      // If no points are within 50px, show all points
      [thumbTip, indexTip, middleTip, ringTip, pinkyTip].forEach((landmark, index) => {
        positionLandmarkDiv(landmark, `point-${index}`, `point-${index}`, `${index + 1}`);
      });
    }
  }
