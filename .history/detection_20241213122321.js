...
document.addEventListener("DOMContentLoaded", function() {
  const path = document.querySelector("#underline");
  const pathLength = path.getTotalLength();  // Get the length of the path

  // Set the stroke dash array and offset based on the path length
  path.style.strokeDasharray = pathLength + " " + pathLength;  // Set dash length to cover the full path
  path.style.strokeDashoffset = pathLength;  // Initially hide the path

  const arrowhead = document.querySelector("#arrowhead");

  // Move the arrowhead along the path using the offset-path property
  arrowhead.style.offsetPath = `path("${path.getAttribute("d")}")`;
  arrowhead.style.offsetDistance = "0%";  // Initially at the start of the path
});
