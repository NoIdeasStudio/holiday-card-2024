...
function resizeVideo() {
  ...
  // Apply media query-like logic for window sizes
  if (windowWidth <= 768) {
    // If window width is less than or equal to 768px (mobile devices)
    // Resize video for smaller screens, make it fill the width
    videoElement.style.width = 'auto';
    videoElement.style.height = '100vh';
  } else if (windowWidth > 768 && windowWidth <= 1024) {
    // If window width is between 768px and 1024px (tablets)
    // Resize video for medium screens, adjust based on aspect ratio
    ...
    // For larger screens (desktops, laptops, etc.)
    // Apply the default behavior based on video aspect ratio
    if (windowWidth / windowHeight > videoAspectRatio) {
      videoElement.style.width = '100vw';
      videoElement.style.height = 'auto';
    } else {
